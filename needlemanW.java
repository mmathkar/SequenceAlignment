public class needlemanW {
	public static void main(String[] args) {
		...
		//the value of match will be returned from scoring matrix function
		...
		for (int i = 1; i <= sequenceA.length(); i++) {
		    for (int j = 1; j <= sequenceB.length(); j++) {
		        ...
		        // we take the minimum
		        opt[i][j] = Math.min(Math.min(scoreDiag, scoreLeft), scoreUp);//change to max
		    }
		}
